...
class SingleTrackDynamics:
    ...
    def loss(self, state, action, ref, dims, factors):
...
class MultiBodyDynamics:
    ...
    def __call__(self, state, action, dt):
        ...
        for i in range(len(f)):
            new_states.append(state[..., i] + f[i].clone().detach().requires_grad_(False) * dt)
        next_state = torch.stack(new_states, dim=-1)

        return next_state

    def loss(self, state, action, ref, dims, factors):
        Rd = [10, 10]  # input difference cost matrix
        Q = [10, 10, 10000, 10000, 10, 10]
        loss = 0
        for i in range(len(Q)):
            loss += Q[i] * (state[..., dims[i]] - ref[..., i])**2
        for i in range(len(Rd)):
