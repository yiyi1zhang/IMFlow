...
def prediction(model, plant_ode, y0, t, p, y, factor, mins, n_samples):
    ...
    # y is tensor in the shape of (seq_len less, y_dim)
    # y_interp in the shape of (seq_len more, y_dim)
    r = int(model.win // model.horizon) if model.win % model.horizon == 0 else \
        int(model.win // model.horizon + 1)
    for k in range(y.shape[0]):
        if k == 0:
            _y_past = torch.repeat_interleave(y_plant[0], repeats=model.win, dim=0)
        elif 0 < k < r:
            _y_past_interp = Interp1D(t[:(k+1)], torch.cat(y_plant[:(k+1)], dim=0).transpose(1, 0), method='linear')
            _y_past1 = _y_past_interp(t1[:k*model.horizon]).transpose(1, 0)
            _y_past = torch.cat([torch.repeat_interleave(y_plant[0], repeats=model.win - model.horizon, dim=0), _y_past1], dim=0)
        else:
            _y_past_interp = Interp1D(t[k-r:k+1], torch.cat(y_plant[k-r:k+1], dim=0).transpose(1, 0), method='linear')
            _y_past = _y_past_interp(t1[(k * model.horizon - model.win):(k * model.horizon)]).transpose(1, 0)

        _y = y_interp[(k * model.horizon):(k + 1) * model.horizon, :] if k * model.horizon < seq_len - 1 \
            else torch.repeat_interleave(y_interp[-1, :].unsqueeze(0), repeats=model.horizon, dim=0)
        _us, _up, _, _ct = prediction_iteration(model, _y_past.unsqueeze(0), _y.unsqueeze(0), n_samples)
        cts.append(_ct)
        u_samples.append(_us)
        u_pred.append(_up)

        if k > 0:
            y_plant.append(update_plant(plant_ode, y0, np.linspace(tStart, (k+1)*tStep, k+2), p, torch.cat(u_pred, dim=0), factor, mins,
                                        [i*tPred for i in range(k+2)]))
        else:
            y_plant.append(y_plant[0])
...
def prediction_iteration(model, y_past, y, n_samples):
    y_past += noise * torch.randn_like(y_past)
    stime = time.time()
    us, y_pred = model.test(y_past, y, n_samples)
    etime = time.time()
    u_mean = us.mean(0).squeeze(0).mean(0, keepdims=True)
    return us.squeeze(1), u_mean, y_pred.squeeze(0), etime-stime


def update_plant(plant_ode, x0, t, p, u, factor, mins, ts=None):
    u = u.detach().numpy()
    u = denormalize(u, factor[y_dim:], mins[y_dim:])
    sol = odeint(plant_ode, x0, t, args=(p, u, ts))
    # ynew = sol[:, :y_dim]
    ynew = sol[-1].reshape(1, -1)[:, selected_inds]
    ynew, _ = normalize(ynew, factor[:y_dim], mins[:y_dim])
...
if __name__ == '__main__':
    ...
    for index in range(n_test):
        ...
        plot_signals(np.linspace(tStart, 3, seq_len), Y[index, ::horizon, :], y_plant[index], U[index, ::horizon, :], u_pred[index], 
                        np.linspace(tStart, 3, seq_len_1), u_samples[index], factor, mins, labels, 
